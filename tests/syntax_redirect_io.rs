...
fn parse_redirect_io() {
    ...
    assert!(matches!(func.body[0], Stmt { kind: StmtKind::Let { .. }, .. }));

    // stmt1: with redirect { stdout: file(f) } { print("out") }
    if let Stmt { kind: StmtKind::WithRedirect { stdout, stderr, stdin, body }, .. } = &func.body[1] {
        ...
        assert!(matches!(body[0], Stmt { kind: StmtKind::Print(_), .. }));
    } else {
        panic!("Expected WithRedirect (stdout -> file)");
    }

    // stmt2: with redirect { stderr: file(f, append=true) } { print_err("err") }
    if let Stmt { kind: StmtKind::WithRedirect { stdout, stderr, stdin, body }, .. } = &func.body[2] {
        assert!(stdin.is_none());
        ...
        assert!(matches!(body[0], Stmt { kind: StmtKind::PrintErr(_), .. }));
    } else {
        panic!("Expected WithRedirect (stderr append)");
    }

    // stmt3: run("cat", f)
    assert!(matches!(func.body[3], Stmt { kind: StmtKind::Run(_), .. }));

    // stmt4: with redirect { stderr: stdout } { print_err("e2s") }
    if let Stmt { kind: StmtKind::WithRedirect { stdout, stderr, stdin, body }, .. } = &func.body[4] {
        assert!(stdin.is_none());
        assert!(stdout.is_none());
        match stderr {
    ...
    assert!(matches!(func.body[5], Stmt { kind: StmtKind::Let { .. }, .. }));

    // stmt6: sh("echo hello > $infile")
    assert!(matches!(func.body[6], Stmt { kind: StmtKind::Sh(_), .. }));

    // stmt7: with redirect { stdin: file(infile) } { run("cat") }
    if let Stmt { kind: StmtKind::WithRedirect { stdout, stderr, stdin, body }, .. } = &func.body[7] {
        assert!(stdout.is_none());
        assert!(stderr.is_none());
        match stdin {
            Some(RedirectTarget::File { path, append }) => {
        ...
        assert!(matches!(body[0], Stmt { kind: StmtKind::Run(_), .. }));
    } else {
        panic!("Expected WithRedirect (stdin <- file)");
    }

    // stmt8: let f2 = capture("mktemp")
    assert!(matches!(func.body[8], Stmt { kind: StmtKind::Let { .. }, .. }));

    // stmt9: with redirect { stdout: stderr, stderr: file(f2) } { print("swap_ok") }
    if let Stmt { kind: StmtKind::WithRedirect { stdout, stderr, stdin, body }, .. } = &func.body[9] {
        assert!(stdin.is_none());
        match stdout {
    ...
    // stmt10: run("cat", f2)
    assert!(matches!(func.body[10], Stmt { kind: StmtKind::Run(_), .. }));

    // cleanup runs exist
    assert!(matches!(func.body[11], Stmt { kind: StmtKind::Run(_), .. }));
    assert!(matches!(func.body[12], Stmt { kind: StmtKind::Run(_), .. }));
    assert!(matches!(func.body[13], Stmt { kind: StmtKind::Run(_), .. }));
}
