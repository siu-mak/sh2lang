...
fn verify_formatting(fixture_name: &str) {
    ...
    assert_eq!(formatted.trim(), expected.trim(), "Formatted output mismatch");
    ...
    let mut formatted_ast = parser::parse(&tokens_fmt, &sm_fmt, "test").expect("Parser failed on formatted output");
