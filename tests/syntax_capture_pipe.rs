...
fn parse_capture_pipe() {
    ...
    // stmt0: let out = capture(run(...) | run(...)) => Expr::CommandPipe([...])
    if let Stmt::Let { name, value } = &func.body[0] {
        assert_eq!(name, "out");
        if let Expr::CommandPipe(segments) = value {
            assert_eq!(segments.len(), 2);

            // seg0
            if let Expr::Literal(ref s) = segments[0][0] {
                 assert_eq!(s, "printf");
            } else { panic!("Expected printf"); }

            // seg1
            if let Expr::Literal(ref s) = segments[1][0] {
                 assert_eq!(s, "sed");
