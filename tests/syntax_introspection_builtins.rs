...
fn parse_introspection_builtins() {
    ...
    if let Stmt { kind: StmtKind::WithEnv { bindings, body }, .. } = &func.body[0] {
        assert_eq!(bindings.len(), 1);
        assert_eq!(bindings[0].0, "FOO");
        assert!(matches!(bindings[0].1, Expr { kind: ExprKind::Literal(ref s), .. } if s == "bar"));
        assert!(matches!(body[0], Stmt { kind: StmtKind::If { .. }, .. }));
    } else {
    ...
    if let Stmt { kind: StmtKind::If { cond, .. }, .. } = &func.body[1] {
        if let Expr { kind: ExprKind::Compare { left, op, right }, .. } = cond {
            ...
            assert!(matches!(**left, Expr { kind: ExprKind::Len(_), .. }));
        } else {
    ...
    if let Stmt { kind: StmtKind::If { cond, .. }, .. } = &func.body[4] {
    ...
    if let Stmt { kind: StmtKind::If { cond, .. }, .. } = &func.body[5] {
        if let Expr { kind: ExprKind::Compare { left, op, right }, .. } = cond {
            ...
            assert!(matches!(**left, Expr { kind: ExprKind::Uid, .. }));
            assert!(matches!(**right, Expr { kind: ExprKind::Number(0), .. }));
