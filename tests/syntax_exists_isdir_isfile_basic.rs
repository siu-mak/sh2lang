...
fn parse_exists_isdir_isfile_basic() {
    ...
    if let Stmt::If { cond, .. } = &func.body[1] {
        ...
        fn contains_exists(e: &Expr) -> bool {
            match e {
                Expr::Exists(_) => true,
                Expr::And(a,b) | Expr::Or(a,b) => contains_exists(a) || contains_exists(b),
                Expr::Not(x) => contains_exists(x),
                Expr::Compare{left,right,..} => contains_exists(left) || contains_exists(right),
                _ => false,
            }
        }
        fn contains_is_file(e: &Expr) -> bool {
            match e {
                Expr::IsFile(_) => true,
                Expr::And(a,b) | Expr::Or(a,b) => contains_is_file(a) || contains_is_file(b),
                Expr::Not(x) => contains_is_file(x),
                Expr::Compare{left,right,..} => contains_is_file(left) || contains_is_file(right),
                _ => false,
            }
        }
        fn contains_is_dir(e: &Expr) -> bool {
            match e {
                Expr::IsDir(_) => true,
                Expr::And(a,b) | Expr::Or(a,b) => contains_is_dir(a) || contains_is_dir(b),
                Expr::Not(x) => contains_is_dir(x),
                Expr::Compare{left,right,..} => contains_is_dir(left) || contains_is_dir(right),
                _ => false,
            }
        }
