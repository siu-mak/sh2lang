...
fn parse_fs_predicates_basic() {
    ...
    if let Stmt::If { cond, .. } = &func.body[1] {
        fn contains_is_symlink(e: &Expr) -> bool {
            match e {
                Expr::IsSymlink(_) => true,
                Expr::And(a,b) | Expr::Or(a,b) => contains_is_symlink(a) || contains_is_symlink(b),
                Expr::Not(x) => contains_is_symlink(x),
                _ => false,
            }
        }
        fn contains_is_exec(e: &Expr) -> bool {
            match e {
                Expr::IsExec(_) => true,
                Expr::And(a,b) | Expr::Or(a,b) => contains_is_exec(a) || contains_is_exec(b),
                Expr::Not(x) => contains_is_exec(x),
                _ => false,
            }
        }
        fn contains_is_readable(e: &Expr) -> bool {
            match e {
                Expr::IsReadable(_) => true,
                Expr::And(a,b) | Expr::Or(a,b) => contains_is_readable(a) || contains_is_readable(b),
                Expr::Not(x) => contains_is_readable(x),
                _ => false,
            }
        }
        fn contains_is_writable(e: &Expr) -> bool {
            match e {
                Expr::IsWritable(_) => true,
                Expr::And(a,b) | Expr::Or(a,b) => contains_is_writable(a) || contains_is_writable(b),
                Expr::Not(x) => contains_is_writable(x),
                _ => false,
            }
        }
